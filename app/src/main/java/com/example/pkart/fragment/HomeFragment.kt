...
import androidx.navigation.fragment.findNavController  // ✅ Import added
import androidx.recyclerview.widget.LinearLayoutManager
import com.bumptech.glide.Glide
import com.example.pkart.R  // ✅ Import added
...
class HomeFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        if (preference.getBoolean("isCart", false)) {
            findNavController().navigate(R.id.action_homeFragment_to_cartFragment) // ✅ Fixed
        }

        val editor = preference.edit()
        editor.putBoolean("isCart", false)  // ✅ Fix: Add an actual edit
    ...
    private fun getSliderImage() {
        Firebase.firestore.collection("slider").document("item")
            .get().addOnSuccessListener {
                val imageUrl = it.getString("img")
                if (!imageUrl.isNullOrEmpty()) {
                    Glide.with(requireContext()).load(imageUrl).into(binding.sliderImage)
                }
            }
            .addOnFailureListener {
                Toast.makeText(requireContext(), "Failed to load slider image", Toast.LENGTH_SHORT).show()
            }
    }
